...
<<主審/Referee, 主審>>は<<ハンドラー/Robot Handler, ハンドラー>>に希望する<<ロボット外装の色/Hull Color of Robots, ロボット外装の色>> (暗もしくは明)を確認する。合意を得られれば、各チームが希望した通りの色を使用する。そうでなければ、主審が任意に色を割り当てる。 +
The <<主審/Referee, referee>> asks the <<ハンドラー/Robot Handler, robot handlers>> for their preferred <<ロボット外装の色/Hull Color of Robots, robot hull color>> (dark or bright). If teams agree, the colors are used; otherwise, the referee assigns them by chance.

ゲーム中を通じて、ロボットは割り当てられた色を維持しなければならない。 +
